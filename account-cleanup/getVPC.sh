...
# filename=vpcid.txt
# rm ./$filename
# aws ec2 describe-vpcs --output text --filters "Name=tag:livefire-lab,Values=vmc-aws" \
#                       --query "Vpcs[].VpcId" | sed -E 's/\s+/\n/g' > $filename
# code $filename
rm ./sg.list.txt
rm ./ig.list.txt
for vpcID in $(aws ec2 describe-vpcs  \
  ...
  aws ec2 describe-internet-gateways --filters 'Name=attachment.vpc-id,Values='$vpcID
        ...
        for GroupID in $(aws ec2 describe-security-groups \
                ...
                do
                 ...
                 echo "Deleting: " $GroupID "For VPC: " $vpcID
